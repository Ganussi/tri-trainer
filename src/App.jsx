...
// ─── Program structure ─────────────────────────────────────────────────────────
// 6-day repeating cycle:
//   Day 1 → Strength Push/Pull
//   Day 2 → Cycling Zone 2
//   Day 3 → Swimming Technique (rotating stroke)
//   Day 4 → Strength Lower Body
//   Day 5 → Cycling Intervals
//   Day 6 → Swimming Endurance
// Total programme: 12 weeks = 72 training days

const TOTAL_DAYS = 72;
const CYCLE = 6;

// Which week number does a given training day fall in? (1-indexed)
function weekOf(day) { return Math.ceil(day / CYCLE); }
// Position in the 6-day cycle (1–6)
function cyclePos(day) { return ((day - 1) % CYCLE) + 1; }

// ─── Stroke rotation ───────────────────────────────────────────────────────────
// Wednesday (pos 3) rotation: Back → Fly → Breast → Fly (repeating)
const STROKE_ROTATION = ["back", "fly", "breast", "fly"];
function strokeForSwimTech(day) {
  // Count how many technique sessions have occurred up to and including this one
  const techSessionIndex = Math.ceil(day / CYCLE) - 1; // 0-indexed week
...
// ─── Swim pace zones (CSS ~1:53–1:54/100m) ────────────────────────────────────
const PACE = {
  z1:   "2:10+/100m",       // easy, warm-up, cool-down, mixed strokes
  z2:   "2:00–2:05/100m",   // steady aerobic
  z3:   "1:58–2:05/100m",   // 80–85% effort
  z4_50:"55–57s/50m",       // 90–95% effort (short reps)
  css:  "1:53–1:54/100m",   // critical swim speed
  neg:  "start 2:05 → finish 1:52/100m", // negative split target
};
...
// ─── Storage ───────────────────────────────────────────────────────────────────
const STORE_KEY = "tri_v3";
function loadStore() {
  try { return JSON.parse(localStorage.getItem(STORE_KEY) || "null"); } catch {}
  return null;
}
...
export default function App() {
  const [store, setStore] = useState(() => loadStore() || DEFAULT);
  ...
  // Streak: consecutive completed days counting back from currentDay-1
  const streak = (() => {
    let s = 0, d = currentDay - 1;
    while (d >= 1) {
      const l = logs[d];
      if (!l || l.skipped) break;
      s++; d--;
    }
    return s;
  })();
  ...
  // Recent 6 days of the cycle to show as strip (days around currentDay)
    ...
    <div style={{ fontFamily: "'Barlow Condensed','Impact',sans-serif", background: "#0a0a0a", minHeight: "100vh", color: "#fff", maxWidth: 430, margin: "0 auto" }}>
      ...
      <div style={{ padding: "52px 20px 16px", background: "linear-gradient(180deg,#111 0%,#0a0a0a 100%)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div>
            <div style={{ fontSize: 11, letterSpacing: 3, color: "#888", fontFamily: "Barlow,sans-serif", textTransform: "uppercase" }}>
            ...
            {tab === "today" && (
              <div style={{ fontSize: 13, color: "#888", fontFamily: "Barlow,sans-serif", marginTop: 2 }}>
                Day {currentPos} of 6 in cycle · {session.label}
              </div>
            )}
          </div>
          ...
          {stripDays.map(d => {
            ...
            return (
              <button key={d} onClick={() => goToDay(d)} style={{ flex: 1, background: "none", border: "none", padding: 0 }}>
                <div style={{ fontSize: 9, color: isCurrent ? "#fff" : "#777", fontFamily: "Barlow,sans-serif", marginBottom: 4, letterSpacing: 1, textAlign: "center" }}>
                  D{d}
                </div>
                <div style={{
                  height: 38, borderRadius: 8,
                  background: done ? col : skipped ? "#1e1e1e" : isCurrent ? col + "33" : "#181818",
                  border: isCurrent ? `2px solid ${col}` : "2px solid transparent",
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "Barlow,sans-serif", fontSize: 11, color: "#777", marginBottom: 5 }}>
            <span>{doneLogs.length} sessions done</span>
      ...
      <div style={{ padding: "0 20px 110px", overflowY: "auto", maxHeight: "calc(100vh - 210px)" }}>
        ...
        {tab === "today" && (
          <div className="fu">
              ...
              <div style={{ background: `linear-gradient(135deg,${session.color}22 0%,transparent 60%)`, padding: "20px 20px 16px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    {session.strokeFocus && (
                      <div style={{ marginTop: 10, display: "flex", flexDirection: "column", gap: 5 }}>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#ffffff0a", borderRadius: 8, padding: "5px 10px" }}>
                          ...
                          <span style={{ fontFamily: "Barlow,sans-serif", fontSize: 13, color: "#e0e0e0" }}>
                            Stroke: <b style={{ color: "#fff" }}>{session.strokeFocus}</b>
                          </span>
                        </div>
                        {session.flyStatus && (
                  ...
                  {todayLogged && !todayLogged.skipped && (
                    <div style={{ background: session.color, borderRadius: 20, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "#000", flexShrink: 0 }}>DONE ✓</div>
                  )}
                  {todayLogged?.skipped && (
                    <div style={{ background: "#2a2a2a", borderRadius: 20, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "#aaa", flexShrink: 0 }}>SKIPPED</div>
                  )}
                </div>
              </div>
            ...
            {showCues && session.strokeCues && (
              <div style={{ background: "#111", borderRadius: "0 0 10px 10px", padding: "12px 16px", border: "1px solid #2a2a2a", borderTop: "none", marginBottom: 8 }}>
                {session.strokeCues.map((c, i) => (
                  <div key={i} style={{ fontFamily: "Barlow,sans-serif", fontSize: 13, color: "#c8c8c8", padding: "5px 0", borderBottom: i < session.strokeCues.length - 1 ? "1px solid #1e1e1e" : "none" }}>
                    · {c}
                  </div>
                ))}
              </div>
            ...
            {/* Pace reference card for swim sessions */}
            {session.type === "swimming" && (
              <div style={{ marginTop: 8, background: "#001a0e", borderRadius: 12, padding: "12px 16px", border: "1px solid #00FF9D18" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  {[
                    { label: "Z1 Easy", pace: "2:10+", color: "#888" },
                    { label: "Z2 Steady", pace: "2:00–2:05", color: "#00D4FF" },
                    { label: "Z3 Threshold", pace: "1:58–2:05", color: "#00FF9D" },
                    { label: "Z4 Speed", pace: "55–57s/50m", color: "#FF6B35" },
            ...
            {!todayLogged && (
              <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                <button onClick={() => setShowLog(true)} style={{ flex: 1, padding: "16px", background: session.color, border: "none", borderRadius: 12, fontSize: 17, fontWeight: 900, color: "#000", letterSpacing: 1, fontFamily: "Barlow Condensed,sans-serif" }}>
                  LOG SESSION
                </button>
                <button onClick={() => logSession({ feel: 3, quick: true })} style={{ padding: "16px", background: "#1e1e1e", border: "1px solid #333", borderRadius: 12, fontSize: 14, fontWeight: 700, color: "#c8c8c8", fontFamily: "Barlow Condensed,sans-serif" }}>
                  QUICK ✓
                </button>
                <button onClick={skipSession} style={{ padding: "16px", background: "#1e1e1e", border: "1px solid #333", borderRadius: 12, fontSize: 14, fontWeight: 700, color: "#888", fontFamily: "Barlow Condensed,sans-serif" }}>
                  SKIP
                </button>
              </div>
            )}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 20, padding: "14px 16px", background: "#161616", borderRadius: 12 }}>
              <button onClick={() => goToDay(currentDay - 1)} disabled={currentDay <= 1}
                style={{ width: 36, height: 36, borderRadius: 8, background: "#222", border: "1px solid #333", color: currentDay <= 1 ? "#333" : "#fff", fontSize: 18 }}>←</button>
              ...
              <button onClick={() => goToDay(currentDay + 1)} disabled={currentDay >= TOTAL_DAYS}
                style={{ width: 36, height: 36, borderRadius: 8, background: "#222", border: "1px solid #333", color: currentDay >= TOTAL_DAYS ? "#333" : "#fff", fontSize: 18 }}>→</button>
            </div>
          </div>
        )}
        ...
        {tab === "schedule" && (
          <div className="fu">
            <div style={{ marginTop: 16, marginBottom: 20, fontFamily: "Barlow,sans-serif", fontSize: 13, color: "#888", lineHeight: 1.5 }}>
              The 6-day cycle repeats for 12 weeks. Rest whenever you need — just pick up from where you left off.
            </div>
              ...
              {CYCLE_LABELS.map((label, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 0", borderBottom: i < 5 ? "1px solid #1e1e1e" : "none" }}>
                    <div style={{ width: 28, height: 28, borderRadius: 8, background: isCurrent ? typeColor + "33" : "#1e1e1e", border: isCurrent ? `1.5px solid ${typeColor}` : "1.5px solid transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>
                      {CYCLE_EMOJIS[i]}
                    </div>
                    <div style={{ flex: 1 }}>
              ...
              {Array.from({ length: 12 }, (_, i) => {
                ...
                return (
                  <div key={week} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: week < 12 ? "1px solid #ffffff05" : "none", opacity: isPast ? 0.4 : 1 }}>
                    ...
                    {stroke === "fly" && (
                      <span style={{ fontFamily: "Barlow,sans-serif", fontSize: 11, color: week <= 2 ? "#FBBF24" : "#00FF9D" }}>
                        {week <= 2 ? "drills only" : "full stroke"}
                      </span>
                    )}
        ...
        {tab === "progress" && (
          <div className="fu">
            ...
            <div style={{ marginTop: 20 }}>
              ...
              {[
                { type: "cycling", label: "Cycling 🚴", positions: [2, 5] },
                { type: "swimming", label: "Swimming 🏊", positions: [3, 6] },
                { type: "strength", label: "Strength 💪", positions: [1, 4] },
              ].map(({ type, label, positions }) => {
                const done = doneLogs.filter(l => positions.includes(l.pos)).length;
                const max = 24;
                return (
                  <div key={type} style={{ marginBottom: 14 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "Barlow,sans-serif", fontSize: 13, marginBottom: 6 }}>
                      ...
                      <span style={{ color: TYPE_COLORS[type] }}>{done} / {max}</span>
                    </div>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ background: "#161616", borderRadius: 12, padding: 14 }}>
                {[
                  { key: "back", label: "Backstroke", emoji: "🔵" },
                  { key: "breast", label: "Breaststroke", emoji: "🟢" },
                  { key: "fly", label: "Butterfly", emoji: "🟡" },
                ].map(({ key, label, emoji }) => {
                  const meta = STROKE_META[key];
                  const total = key === "fly" ? 6 : 3;
                  ...
                  return (
                    <div key={key} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0", borderBottom: key !== "fly" ? "1px solid #1e1e1e" : "none" }}>
                      ...
                      <div style={{ background: meta.color + "22", borderRadius: 8, padding: "4px 10px", fontFamily: "Barlow,sans-serif", fontSize: 13, color: meta.color, fontWeight: 700 }}>
                        {done}/{total}
                      </div>
                    </div>
                  );
            ...
            <div style={{ marginTop: 20 }}>
              ...
              {allLogs.length === 0 && (
                <div style={{ fontFamily: "Barlow,sans-serif", fontSize: 14, color: "#555", textAlign: "center", padding: "24px 0" }}>No sessions logged yet. Let's go! 💪</div>
              )}
              {allLogs.slice(-6).reverse().map((log, i) => {
                ...
                return (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid #1a1a1a" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: "Barlow,sans-serif", fontSize: 13, color: log.skipped ? "#777" : "#e0e0e0" }}>
                        {log.skipped ? "Skipped" : s.title}
                      </div>
...
function MiniChart({ data, color, unit, lowerIsBetter }) {
  ...
  const pts = data.map((d, i) => ({ x: pad + (i / Math.max(data.length-1,1)) * (W-pad*2), y: H-pad-((d.value-min)/range)*(H-pad*2), d }));
